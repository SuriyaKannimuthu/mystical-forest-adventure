...
public class GameManager : MonoBehaviour
{
    ...
    private IEnumerator SendSignal()
    {
        ...
        apiform = new APIForm
        {
            status = true, // Assuming the spin was successful
            winpaylines = new int[] { 1, 5, 9 }, // Example paylines where the player won
            randomNums = new int[]
    {
        // First reel (3 symbols)
        5, 9, 14, 
        // Second reel (3 symbols)
        7, 3, 10, 
        // Third reel (3 symbols)
        12, 8, 1, 
        // Fourth reel (3 symbols)
        6, 15, 11, 
        // Fifth reel (3 symbols)
        13, 4, 2
    }, // 15 random numbers for the reels
            moneyResult = 500, // Example win amount
            message = "Congratulations! You won 500 coins!" // Example message
        };
        ...
        for (int i = 0; i < 5; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                ...
                try
                {
                    ...
                    var obj = Instantiate(_winningIcons[index], _winningSlots[j + i * 3]);
